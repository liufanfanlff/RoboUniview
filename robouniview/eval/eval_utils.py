...
from robouniview.data.data import preprocess_image, preprocess_text_calvin
...
class ModelWrapper(CalvinBaseModel):
    ...
    def step(self, obs, goal,env, get_action=True):
        ...
        calib = (static_extrinsic_matrix,static_intrinsic_matrix,static_distCoeffs_matrix,
                 gripper_extrinsic_matrix,gripper_intrinsic_matrix,gripper_distCoeffs_matrix,gripper_extrinsic_matrix)
        ...
        with torch.no_grad():
            ...
            if self.use_diff:
                ...
                if self.fusion_mode == 'two_way':
                    ...
                    action,_ = self.model(vision_x=image_x, lang_x=text_x, attention_mask=mask, vision_gripper = gripper, state_tensor = state,calib = calib, return_feature=True)
    ...
    for step in range(EP_LEN):
        if model.replan != -1 and step % model.replan == 0:
            if model.model.module.refresh != -1:
                ...
                model.reset()
        action = model.step(obs, lang_annotation,env, (len(planned_actions) == 0))

        if model.model.module.occ_loss:
...
def eval_one_epoch_calvin_with_dataloder(args, model, calvin_loader, image_processor, tokenizer, eval_log_dir=None, debug=False, future_act_len=-1, reset=False, diverse_inst=False):
    ...
    for num_steps, batch_calvin in t:
        ...
        while not dt['done']:
            ...
            action = wrapped_model.step(obs, lang_annotation, wrapped_model.model.module.env, (len(planned_actions) == 0))
            if len(planned_actions) == 0:
                if action.shape == (7,):
                    planned_actions.append(action)
                else:
