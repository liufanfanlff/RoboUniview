...
def train_one_epoch_calvin(
    ...
    for num_steps, batch_calvin in t:
        ...
        if args.train_action:
            ...
            loss_calvin_num = torch.tensor(0).to(loss_occ['grid_cls_occ_loss'].device)
            loss_calvin_bin = torch.tensor(0).to(loss_occ['grid_cls_occ_loss'].device)
        if args.real_data:
        ...
        for k in loss_occ.keys():
            loss += loss_occ[k]*args.loss_weight['occ']

        # if num_steps%100 == 0 and args.occ_loss:
        #     with torch.no_grad():
        ...
        #         o3d.io.write_point_cloud("pcd_true.pcd", pcd_true)
        ...
        #loss_dic = {"avg loss": sum(mv_avg_loss[-avg_horizon:]) / avg_horizon, "loss": loss_calvin.item(), "Lnum": loss_calvin_num.item(), "Lbin": loss_calvin_bin.item()}
        for k in loss_occ.keys():
